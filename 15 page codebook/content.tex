...
u64 pollard(u64 n) {              // one non-trivial factor of composite n
    if (n % 2 == 0) return 2;
    for (u64 c = 1;; c++) {
        ...
        if (d != n) return d;     // else retry with the next c
    }
}
void factor(u64 n, vector<u64> &out) {   // full factorisation, unsorted
    if (n == 1) return;
...
\begin{lstlisting}
// maximum of a unimodal f on [l, r]; flip the < for the minimum
...
\end{lstlisting}
\subsection{Monotonic Stack (next greater)}
\begin{lstlisting}
// needs: stack<int> stk; vector<int> v(n), ind(n);
// ind[i] = index of the next strictly greater element to the right
for (int i = n - 1; i >= 0; i--) {
...
// vector<int> moves = {1,2,3}; vector<int> dp(n+1, -1);
// grundy(n, moves, dp);  XOR the grundy values of independent piles
\end{lstlisting}
\subsection{Manacher}
\begin{lstlisting}
struct Manacher {
...
\begin{lstlisting}
int fenwick[N];                  // 1-indexed
void update(int ind, int val) {  // add val at ind
    while (ind < N) { fenwick[ind] += val; ind += ind & -ind; }
}
int query(int ind) {             // prefix sum [1, ind]
    int sum = 0;
    while (ind > 0) { sum += fenwick[ind]; ind -= ind & -ind; }
    return sum;
}
...
// read a[1..n][1..n], then call maxmatrix(n)
\end{lstlisting}
\subsection{SOS DP}
\begin{lstlisting}
...
\subsection{Digit DP}
\begin{lstlisting}
// count the values in [L, R] with some property: f(R) - f(L - 1)
string s;              // the upper bound, as a digit string
int dp[20][2][2];      // pos, tight, started
int f(int pos, int tight, int started) {
    if (pos == (int)s.size()) return started;   // <- the property goes here
...
\end{lstlisting}
\subsection{SCC + Condensation}
\begin{lstlisting}
// components = the SCCs, adj_cond = condensation graph
// (its vertices are named by the smallest vertex of each SCC)
vector<bool> visited;
void dfs(int v, vector<vector<int>> const &adj, vector<int> &out) {
    visited[v] = true;
    for (auto u : adj[v])
        if (!visited[u]) dfs(u, adj, out);
    out.push_back(v);
}
void scc(vector<vector<int>> const &adj, vector<vector<int>> &components,
         vector<vector<int>> &adj_cond) {
    int n = adj.size();
    components.clear(), adj_cond.clear();
    vector<int> order;               // vertices by exit time
    visited.assign(n, false);
    for (int i = 0; i < n; i++)
        if (!visited[i]) dfs(i, adj, order);
    vector<vector<int>> adj_rev(n);  // transpose
    for (int v = 0; v < n; v++)
        for (int u : adj[v]) adj_rev[u].push_back(v);
    visited.assign(n, false);
    reverse(order.begin(), order.end());
    vector<int> roots(n, 0);         // root of each vertex's SCC
    for (auto v : order)
        if (!visited[v]) {
